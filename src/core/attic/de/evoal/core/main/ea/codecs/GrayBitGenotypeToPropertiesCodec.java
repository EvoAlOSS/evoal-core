...
import de.evoal.core.main.ea.chromosomes.GrayBoundedBitChromosome;
import de.evoal.core.main.ea.chromosomes.GrayHelper;
...
public class GrayBitGenotypeToPropertiesCodec implements CustomCodec<BitGene> {
	...
	public GrayBitGenotypeToPropertiesCodec(final PropertiesSpecification specification, final List<Pair<Double, Double>> limits, final int scale) {
	...
	public Factory<Genotype<BitGene>> encoding() {
		...
		for (int i=0; i< dimensions; i++) {
			...
			listOfChromosomes.add(GrayBoundedBitChromosome.of(limit.getFirst(), limit.getSecond(), scale));
		}
	...
	private Properties fromChromosome(final Genotype<BitGene> genotype) {
		...
		for(int i = 0; i< genotype.length(); i++) {
			final GrayBoundedBitChromosome chr = (GrayBoundedBitChromosome)genotype.get(i); // nasty girl
			final byte [] bytes = chr.toByteArray();
			
			vector[i] = GrayHelper.fromGray(bytes, scale);
		}
	...
	public Genotype<BitGene> encode(final Properties individuum) {
		...
		for (int i = 0; i < dimensions; i++) {
			final Pair<Double, Double> limit = limits.get(i);
			listOfChromosomes.add(GrayBoundedBitChromosome.of(individuum.getValues()[i], limit.getFirst(), limit.getSecond(), scale));
		}
		return Genotype.of(listOfChromosomes);
	}
}
