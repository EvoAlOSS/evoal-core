...
import de.evoal.core.ea.main.codec.program.operations.DivideOperation;
import de.evoal.core.ea.main.codec.program.operations.SqrtOperation;
...
@Slf4j
public class ConstantSqrtRewriter extends Rewriter<SqrtOperation> {
    public ConstantSqrtRewriter() {
        super(SqrtOperation.class);
    }

    @Override
    public int apply(final TreeNode<Op<Double>> tree, final SqrtOperation content) {
...
//        final Optional<Double> right = toConstantValue(tree.childAt(1));
        ...
        final Op<Double> replacement = Const.of(Math.sqrt(left.get())); // Const.of(Math.pow(right.get(), 1.0 / left.get()));
