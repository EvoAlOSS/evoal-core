...
import de.evoal.core.ea.main.codec.program.operations.PowOperation;
...
@Slf4j
public class ConstantPowRewriter extends Rewriter<PowOperation> {
    public ConstantPowRewriter() {
        super(PowOperation.class);
    }

    @Override
    public int apply(final TreeNode<Op<Double>> tree, final PowOperation content) {
        ...
        final Op<Double> replacement = Const.of(Math.pow(left.get(), right.get()));
