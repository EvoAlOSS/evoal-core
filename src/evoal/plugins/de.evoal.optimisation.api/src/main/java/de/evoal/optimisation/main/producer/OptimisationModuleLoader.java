...
public class OptimisationModuleLoader {
    ...
    public void load(final @Observes BlackboardEntry entry) {
        if(!entry.isSame(OptimisationBlackboardEntries.OPTIMISATION_CONFIGURATION_FILE)) {
            return;
        }

        final String configurationFileName = board.get(OptimisationBlackboardEntries.OPTIMISATION_CONFIGURATION_FILE);
        ...
        try {
            ...
            board.bind(OptimisationBlackboardEntries.OPTIMISATION_CONFIGURATION, model);
